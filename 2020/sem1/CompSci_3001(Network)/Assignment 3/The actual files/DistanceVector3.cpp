...
int main(int argc, char **argv) {
  ...
  int count, cost, time = 0;

  /* Read in data */
  char str1 = fgetc(config_file);
  int num_nodes = str1 - '0';
  ...
  // Assigning the same cell block (As in the index i = j) in the 2d dmat[][] table with '0' because the routers are rounting through itself at these cells. 
...
void display2(char s, char d, char via, int cost) { 
  printf("\nrouter %c: %c is %d routing through %c ", s, d, cost, via); 
}