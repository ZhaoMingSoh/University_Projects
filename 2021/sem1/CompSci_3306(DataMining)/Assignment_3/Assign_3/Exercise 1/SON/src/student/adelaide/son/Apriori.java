...
public class Apriori extends Observable {
	...
	public Apriori(String inFile, double s) throws Exception
	{
		transaFile = inFile;
	...
	public List<String> startPasses() throws Exception {
		itemsets = new ArrayList<int[]>();
		frequentItemsets = new ArrayList<String>();
	...
	private void calculateFrequentItemSets() throws Exception
	{
		...
		for (int i=0;i<itemsets.size();i++)
		{
			if((count[i]/(double)numTransactions) >= minSup) {
				...
				frequentItemsets.add(Arrays.toString(itemsets.get(i)) + ":" + count[i]);
